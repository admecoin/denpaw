...
use Denpaw\Playcoin\Config;
use Denpaw\Playcoin\Exceptions;
use Denpaw\Playcoin\Responses\PlaycoindResponse;
...
class ClientTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->playcoind = new PlaycoinClient();
    }
    ...
    {
        $playcoind = new PlaycoinClient('http://old_client.org');
        $this->assertInstanceOf(PlaycoinClient::class, $playcoind);

        $base_uri = $playcoind->getClient()->getConfig('base_uri');
        $this->assertEquals($base_uri->getHost(), 'old_client.org');

        $oldClient = $playcoind->getClient();
        $this->assertInstanceOf(GuzzleHttp::class, $oldClient);

        $newClient = new GuzzleHttp(['base_uri' => 'http://new_client.org']);
        $playcoind->setClient($newClient);
    ...
    {
        $playcoind = new PlaycoinClient(['preserve_case' => true]);
        $playcoind->setClient($this->mockGuzzle([$this->getBlockResponse()]));
        $playcoind->getBlockHeader();
    ...
    {
        $this->assertInstanceOf(Config::class, $this->playcoind->getConfig());
    }
    ...
    public function testAsyncRequest(): void
    {
        $onFulfilled = $this->mockCallable([
            $this->callback(function (PlaycoindResponse $response) {
    ...
    /**
     * Test playcoind exception.
     *
     * @return void
     */
    public function testPlaycoindException(): void
    {
...
class FakeClient extends PlaycoinClient
{
    /**
    ...
    {
        return 'Denpaw\\Playcoin\\Tests\\FakeResponse';
    }
}
