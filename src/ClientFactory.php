...
class ClientFactory
{
    ...
    public function getConfig(string $name = 'default'): array
    {
        if (isset($this->config['host']) && ! is_array($this->config['host'])) {
            $this->logger->warning(
                'laravel-playcoind: You are using legacy config format which '.
                'was deprecated and will be removed in the next version. '.
                'Please update your config file by running [php artisan '.
                'vendor:publish '.
                '--provider="Denpaw\Playcoin\Providers\ServiceProvider" --force].'
            );
     ...
     * @param  string  $name
     *
     * @return \Denpaw\Playcoin\Client
     */
    public function client(string $name = 'default'): PlaycoinClient
    {
     ...
     * @return \Denpaw\Playcoin\Client
     */
    public function make(array $config = []): PlaycoinClient
    {
        return new PlaycoinClient($config);
    }
